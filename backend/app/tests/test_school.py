...
class TestGetSchool:
    ...
    def test_get_school_found(self, client):
        ...
        client.post("/api/school", json=school_data)
...
class TestCreateSchool:
    ...
    def test_create_school_success(self, client):
        ...
        response = client.post("/api/school", json=school_data)
        assert response.status_code == 201
        data = response.json()
        assert data["name"] == "New School"
        assert data["id"] == 1

    def test_create_school_minimal(self, client):
    ...
    def test_create_school_duplicate_conflict(self, client):
        """Test that creating a second school returns 409 Conflict."""
        school_data = {"name": "First School"}
        response1 = client.post("/api/school", json=school_data)
        assert response1.status_code == 201

        # Try to create a second school
        school_data2 = {"name": "Second School"}
        response2 = client.post("/api/school", json=school_data2)
...
class TestUpdateSchool:
    ...
    def test_update_school_success(self, client):
        ...
        response = client.put("/api/school", json=update_data)
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == "Updated School"
        assert data["description"] == "Updated description"

    def test_update_school_partial(self, client):
        ...
        # Update only phone
        response = client.put("/api/school", json={"phone": "555-2222"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_update_school_empty_request(self, client):
        ...
        # Update with no data
        response = client.put("/api/school", json={})
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == "Test School"
...
class TestDeleteSchool:
    ...
    def test_delete_school_not_found(self, client):
        ...
        response = client.delete("/api/school")
        assert response.status_code == 404
        data = response.json()
        assert data["detail"] == "School not found"

    def test_delete_school_success(self, client):
    ...
    def test_delete_then_create_new(self, client):
        """Test that after deletion, a new school can be created."""
        # Create a school
        client.post("/api/school", json={"name": "First"})

        # Delete it
        client.delete("/api/school")

        # Create a new one (should work since old one is deleted)
        response = client.post("/api/school", json={"name": "Second"})
        assert response.status_code == 201
